...
export class UnityJSLibExportedService {
  ...
  /** Register a handler for RequestDataFromWeb requests from Unity. Used for any instance without its own forInstance() handler. */
  registerRequestDataFromWebHandler(handler: (query: string, respond: (result: string) => void) => void): void {
    requestDataFromWebHandler = handler;
  }
  
  /** Invoke the callback registered by Unity via RegisterOnNavigationChanged. Broadcasts to all instances; use forInstance() to target one. */
  notifyOnNavigationChanged(data: string): void {
    if (channels.size > 0) {
